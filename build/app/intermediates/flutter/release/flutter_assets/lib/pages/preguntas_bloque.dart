...
class _PreguntasBloquePageState extends State<PreguntasBloquePage> {
  ...
  Future<void> _guardarRespuestas() async {
    ...
    try {
      ...
      Map<String, dynamic> dbFincasUpdate = {};
      ...
      for (int i = 0; i < preguntas.length; i++) {
        if (respuestas.containsKey(i)) {
          ...
          // Actualizar dbFincasUpdate si corresponde
          _updateDbFincasFields(
              preguntas[i]['idpreg'], respuesta, dbFincasUpdate);
        }
      }

      // Actualizar dbfincas si hay campos para actualizar
      if (dbFincasUpdate.isNotEmpty) {
        await _updateDbFincas(dbFincasUpdate);
      }
  ...
  void _updateDbFincasFields(int idpreg, Map<String, dynamic> respuesta,
      Map<String, dynamic> dbFincasUpdate) {
    switch (idpreg) {
      case 1:
        dbFincasUpdate['stipo'] = respuesta['strespuesta'];
        break;
      case 2:
        dbFincasUpdate['snit'] = respuesta['strespuesta'];
        break;
      case 3:
        dbFincasUpdate['ntipo_doc'] = respuesta['nnumero'];
        break;
    }
  }

  Future<void> _updateDbFincas(Map<String, dynamic> updateData) async {
    try {
      await OfflineSupport.updateDbFincas(widget.idForm, updateData);
    } catch (e) {
      print('Error al actualizar dbfincas: $e');
      // Guardar la actualización para sincronización posterior
      await OfflineSupport.saveResponseLocally({
        'table': 'dbfincas',
        'action': 'update',
        'knumero': widget.idForm,
        'data': updateData,
      });
    }
  }
